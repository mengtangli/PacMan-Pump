...
plot3(sqrt(3)*R*cos(angle), sqrt(3)*R*sin(angle), 0*angle+R, 'r', 'linewidth',2)
plot3(sqrt(3)*R*cos(angle), sqrt(3)*R*sin(angle), 0*angle-R, 'r', 'linewidth',2)
pause(2);
...
for i = 1:1:n
    ...
    % for copy and paste use only
%     syms w;
%     cosa = 3/4; sina = sqrt(7)/4;
%     A = [cosa*cos(w) -sin(w) -sina*cos(w);
%         cosa*sin(w) cos(w) -sina*sin(w);
%         sina 0 cosa];
%     Ainv = inv(A)
