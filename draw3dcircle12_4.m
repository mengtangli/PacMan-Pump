% Plot moving 3D gear set 3 vertex with 2R-3R gears, no L
% Mengtang Li
% Apr 8
clear; clf; close all;
...
plot3(R*cos(angle), R*sin(angle), 0*angle+sqrt(5)/2*R, 'r', 'linewidth',2)
plot3(R*cos(angle), R*sin(angle), 0*angle-sqrt(5)/2*R, 'r', 'linewidth',2)
pause(2);
...
for i = 1:1:n
    ...
    % cosa = 2/3; sina = sqrt(5)/3;
    Ainv = [ 2*cos(1.5*wt)/3          2*sin(1.5*wt)/3        5^(1/2)/3;
        -sin(1.5*wt)                cos(1.5*wt)                  0;
        -5^(1/2)*cos(1.5*wt)/3    -5^(1/2)*sin(1.5*wt)/3           2/3;];
    ...
    theta = 0:0.02:2*pi;
    rho_x = 1.5*R*(cos(theta)*C(1,1)+sin(theta)*C(2,1));
    rho_y = 1.5*R*(cos(theta)*C(1,2)+sin(theta)*C(2,2));
    rho_z = 1.5*R*(cos(theta)*C(1,3)+sin(theta)*C(2,3));
    % lower part half circle 1st point (0 deg) part
    phi = 0:0.02:pi/2;
    rho_p1_x = 1.5*R*cos(phi)*cos(0)*C(1,1)+1.5*R*cos(phi)*sin(0)*C(2,1)+1.5*R*sin(phi)*C(3,1);
    rho_p1_y = 1.5*R*cos(phi)*cos(0)*C(1,2)+1.5*R*cos(phi)*sin(0)*C(2,2)+1.5*R*sin(phi)*C(3,2);
    rho_p1_z = 1.5*R*cos(phi)*cos(0)*C(1,3)+1.5*R*cos(phi)*sin(0)*C(2,3)+1.5*R*sin(phi)*C(3,3);
    % lower part half circle 2nd point (120 deg) part
    rho_p2_x = 1.5*R*cos(phi)*cos(2*pi/3)*C(1,1)+1.5*R*cos(phi)*sin(2*pi/3)*C(2,1)+1.5*R*sin(phi)*C(3,1);
    rho_p2_y = 1.5*R*cos(phi)*cos(2*pi/3)*C(1,2)+1.5*R*cos(phi)*sin(2*pi/3)*C(2,2)+1.5*R*sin(phi)*C(3,2);
    rho_p2_z = 1.5*R*cos(phi)*cos(2*pi/3)*C(1,3)+1.5*R*cos(phi)*sin(2*pi/3)*C(2,3)+1.5*R*sin(phi)*C(3,3);
    % lower part half circle 3rd point (240 deg) pary
    rho_p3_x = 1.5*R*cos(phi)*cos(4*pi/3)*C(1,1)+1.5*R*cos(phi)*sin(4*pi/3)*C(2,1)+1.5*R*sin(phi)*C(3,1);
    rho_p3_y = 1.5*R*cos(phi)*cos(4*pi/3)*C(1,2)+1.5*R*cos(phi)*sin(4*pi/3)*C(2,2)+1.5*R*sin(phi)*C(3,2);
    rho_p3_z = 1.5*R*cos(phi)*cos(4*pi/3)*C(1,3)+1.5*R*cos(phi)*sin(4*pi/3)*C(2,3)+1.5*R*sin(phi)*C(3,3);
    
    % vertex of normal vector of radius R in moving frame
    rho_xvt = 0+0+1.5*R*C(3,1);
    rho_yvt = 0+0+1.5*R*C(3,2);
    rho_zvt = 0+0+1.5*R*C(3,3);
    % 1st point (0 deg) in moving frame
    rho_x1 = 1.5*R*cos(0)*C(1,1)+1.5*R*sin(0)*C(2,1);
    rho_y1 = 1.5*R*cos(0)*C(1,2)+1.5*R*sin(0)*C(2,2);
    rho_z1 = 1.5*R*cos(0)*C(1,3)+1.5*R*sin(0)*C(2,3);
    % 2nd point (120 deg) in moving frame
    rho_x2 = 1.5*R*cos(2*pi/3)*C(1,1)+1.5*R*sin(2*pi/3)*C(2,1);
    rho_y2 = 1.5*R*cos(2*pi/3)*C(1,2)+1.5*R*sin(2*pi/3)*C(2,2);
    rho_z2 = 1.5*R*cos(2*pi/3)*C(1,3)+1.5*R*sin(2*pi/3)*C(2,3);
    % 3rd point (240 deg) in moving frame
    rho_x3 = 1.5*R*cos(4*pi/3)*C(1,1)+1.5*R*sin(4*pi/3)*C(2,1);
    rho_y3 = 1.5*R*cos(4*pi/3)*C(1,2)+1.5*R*sin(4*pi/3)*C(2,2);
    rho_z3 = 1.5*R*cos(4*pi/3)*C(1,3)+1.5*R*sin(4*pi/3)*C(2,3);
