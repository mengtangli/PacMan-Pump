...
%         -cos(2*wt)/2,            -sin(2*wt)/2,         3^(1/2)/2;];
    ...
    % lowered big half circle [CAYON] (pi+asin(0.1)~2*pi-asin(0.1))
    phi2 = (pi+asin(0.1)):0.02:(2*pi-asin(0.1));
    rho2_xe2 = 3*R*cos(phi2)*C(2,1)+(3*R*sin(phi2)-(h-3*R*0.1))*C(3,1);
    rho2_ye2 = 3*R*cos(phi2)*C(2,2)+(3*R*sin(phi2)-(h-3*R*0.1))*C(3,2);
    rho2_ze2 = 3*R*cos(phi2)*C(2,3)+(3*R*sin(phi2)-(h-3*R*0.1))*C(3,3);
    
    % vertex of normal vector of radius R in moving frame {BLUE}
    rho_xvt = 0+0+2*R*C(3,1);
    rho_yvt = 0+0+2*R*C(3,2);
    rho_zvt = 0+0+2*R*C(3,3);
    % 1st point (0 deg) in moving frame [on BLUE circle]
    rho_x1 = 2*R*(cos(0)*C(1,1)+sin(0)*C(2,1));
    rho_y1 = 2*R*(cos(0)*C(1,2)+sin(0)*C(2,2));
    rho_z1 = 2*R*(cos(0)*C(1,3)+sin(0)*C(2,3));
    % 2nd point (90 deg) in moving frame [on BLUE circle]
    rho_x2 = 2*R*(cos(pi/2)*C(1,1)+sin(pi/2)*C(2,1));
    rho_y2 = 2*R*(cos(pi/2)*C(1,2)+sin(pi/2)*C(2,2));
    rho_z2 = 2*R*(cos(pi/2)*C(1,3)+sin(pi/2)*C(2,3));
    % 3rd point (180 deg) in moving frame [on BLUE circle]
    rho_x3 = 2*R*(cos(pi)*C(1,1)+sin(pi)*C(2,1));
    rho_y3 = 2*R*(cos(pi)*C(1,2)+sin(pi)*C(2,2));
    rho_z3 = 2*R*(cos(pi)*C(1,3)+sin(pi)*C(2,3));
    % 4th point (270 deg) in moving frame [on BLUE circle]
    ...
    % 2nd point (90 deg) in big half cicle [RED]
    ...
    % 2nd point (90 deg) in LOWERED big half cicle [CAYON]
    rho_x2e2 = 3*R*cos(pi+asin(0.1))*C(2,1)+(3*R*sin(pi+asin(0.1))-(h-3*R*0.1))*C(3,1);
    rho_y2e2 = 3*R*cos(pi+asin(0.1))*C(2,2)+(3*R*sin(pi+asin(0.1))-(h-3*R*0.1))*C(3,2);
    rho_z2e2 = 3*R*cos(pi+asin(0.1))*C(2,3)+(3*R*sin(pi+asin(0.1))-(h-3*R*0.1))*C(3,3);
    % 4th point (270 deg) in big half cicle [RED]
    ...
    % 4th point (270 deg) in LOWERED big half cicle [RED]
    rho_x4e2 = 3*R*cos(2*pi-asin(0.1))*C(2,1)+(3*R*sin(2*pi-asin(0.1))-(h-3*R*0.1))*C(3,1);
    rho_y4e2 = 3*R*cos(2*pi-asin(0.1))*C(2,2)+(3*R*sin(2*pi-asin(0.1))-(h-3*R*0.1))*C(3,2);
    rho_z4e2 = 3*R*cos(2*pi-asin(0.1))*C(2,3)+(3*R*sin(2*pi-asin(0.1))-(h-3*R*0.1))*C(3,3);
    % center of moving circle in fixed frame [center of BLUE circle]
    r_x = R_x + rho_x;
    r_y = R_y + rho_y;
    r_z = R_z + rho_z;
    % vertex of normal vector of radius R in fixed frame [BLUE line]
    r_xvt = R_x + rho_xvt;  x_record1(i) = r_xvt;
    r_yvt = R_y + rho_yvt;  y_record1(i) = r_yvt;
    r_zvt = R_z + rho_zvt;  z_record1(i) = r_zvt;
    % 1st point (0 deg) in fixed frame [not shown]
    r_x1 = R_x + rho_x1;  x_record2(i) = r_x1;
    r_y1 = R_y + rho_y1;  y_record2(i) = r_y1;
    r_z1 = R_z + rho_z1;  z_record2(i) = r_z1;
    % 2nd point (90 deg) in fixed frame [GREEN line]
    r_x2 = R_x + rho_x2;  x_record3(i) = r_x2;
    r_y2 = R_y + rho_y2;  y_record3(i) = r_y2;
    r_z2 = R_z + rho_z2;  z_record3(i) = r_z2;
    % 3rd point (180 deg) in fixed frame [not shown]
    r_x3 = R_x + rho_x3;
    r_y3 = R_y + rho_y3;
    r_z3 = R_z + rho_z3;
    % 4th point (270 deg) in fixed frame [GREEN line]
    r_x4 = R_x + rho_x4;
    r_y4 = R_y + rho_y4;
    r_z4 = R_z + rho_z4;
    % lower part half circle [GREEN half circle]
    r2_x = R_x + rho2_x;
    r2_y = R_y + rho2_y;
    r2_z = R_z + rho2_z;
    % half circle [RED half circle]
    r2_xe = R_x + rho2_xe; 
    r2_ye = R_y + rho2_ye;
    r2_ze = R_z + rho2_ze + cos(beta)*h;
    % LOWERED half circle [CAYON half circle]
    r2_xe2 = R_x + rho2_xe2 + 0.3*R*sin(beta); % just translation. same
    r2_ye2 = R_y + rho2_ye2 + 0;         % trajectory.
    r2_ze2 = R_z + rho2_ze2 + cos(beta)*h - 0.3*R*cos(beta);
    ...
    r_x2e2 = R_x + rho_x2e2 + 0.3*R*sin(beta);  x_record5(i) = r_x2e2;
    r_y2e2 = R_y + rho_y2e2 + 0;  y_record5(i) = r_y2e2;
    r_z2e2 = R_z + rho_z2e2 + cos(beta)*h - 0.3*R*cos(beta);  z_record5(i) = r_z2e2;
    % extended 4th point (270 deg) in moving frame
    r_x4e = R_x + rho_x4e;
    r_y4e = R_y + rho_y4e;
    r_z4e = R_z + rho_z4e + cos(beta)*h;
    %!! lowered extended 4th point (270 deg) in moving frame
    r_x4e2 = R_x + rho_x4e2 + 0.3*R*sin(beta);
    r_y4e2 = R_y + rho_y4e2 + 0;
    r_z4e2 = R_z + rho_z4e2 + cos(beta)*h - 0.3*R*cos(beta);
...
plot3(x_record6, y_record6, z_record6, 'k-.','linewidth',1);